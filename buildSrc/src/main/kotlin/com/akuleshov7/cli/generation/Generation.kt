...
package com.akuleshov7.cli.generation
...
private val cliName = "kotlin-mpp-cli"
 ...
 * @property fullName Full name of option for usage in cli
 * @property shortName Short name of option for usage in cli
...
fun Project.generateConfigOptions(destination: File) {
    ...
    generateCliProperties(options, arguments, destination)
    generateReadme(options, File(generatedOptionsTablePath()))
}

/**
 * Generate CliProperties class which represents configuration properties of cli application
 *
 * @param options map of cli option names to [Option] objects
 * @param arguments map of cli argument names to [Argument] objects
 * @param destination
 */
fun generateCliProperties(
    options: Map<String, Option>,
    arguments: Map<String, Argument>,
    destination: File
) {
    ...
    val classBuilder = generateCliPropertiesClass(options, arguments)
    val companion = TypeSpec.companionObjectBuilder()
        .addFunction(generateParseArgsFunc(options, arguments).build())
        .build()
    classBuilder.addType(companion)
    builder.addType(classBuilder.build())
    builder.addType(genereatePropertiesClassWithDefaults("CliProperties", options, arguments).build())
...
/**
 * Generate constructors for CliProperties class
 *
 * @param options map of cli option names to [Option] objects
 * @param arguments map of cli argument names to [Argument] objects
...
fun generateCliPropertiesClass(options: Map<String, Option>, arguments: Map<String, Argument>): TypeSpec.Builder {
    val classBuilder = TypeSpec.classBuilder("CliProperties").addModifiers(KModifier.DATA)
    ...
    val kdoc = """
               |Configuration properties of cli application, retrieved either from properties file
               |or from CLI args.
               |$properties
               """.trimMargin()
...
fun generateParseArgsFunc(options: Map<String, Option>, arguments: Map<String, Argument>): FunSpec.Builder {
    ...
    parseArgsFunc.addStatement("val %N = ArgParser(%S)", "parser", cliName)
...
fun generateReadme(jsonObject: Map<String, Option>, destination: File) {
    var readmeContent =
            """
                |Most (except for `-h` and `-prop`) of the options below can be passed to an app via `.properties` file
                |
